---
name: prepare
description: 将 draft/<代号>/cn-article.md 翻译为英文，并上传草稿图片到 R2，回填 metadata.md 图片路径。用户输入 "/prepare <代号>"、"prepare:<代号>"、"准备草稿:<代号>" 或类似表述时使用。
---
...
例：`/prepare 0606tokenmaxxing`

`<代号>` 从 slash 命令参数中获取（也可用 "prepare:<代号>"、"准备草稿:<代号>" 等自然语言触发），所有文件路径均以 `draft/<代号>/` 为根目录。
